...
public class StatisticClient {
    ...
    private final ru.practicum.client.StatisticClient statsClient;

    public ResponseEntity<Object> saveHit(String uri, String ip) {
        StatisticInDto hitRequestDto = StatisticInDto.builder()
                .app(APP)
                .uri(uri)
                .ip(ip)
    ...
    public EventFullDto setViewsNumber(EventFullDto event) {
        List<StatisticViewDto> hits = statsClient.getStatistic(event.getCreatedOn(), LocalDateTime.now(),
    ...
    public List<EventShortDto> setViewsNumber(List<EventShortDto> events) {
        ...
        List<StatisticViewDto> hits = statsClient.getStatistic(LocalDateTime.now().minusYears(YEARS_OFFSET),
    ...
    private Map<Long, Integer> mapHits(List<StatisticViewDto> hits) {
